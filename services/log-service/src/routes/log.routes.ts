...
router.post('/:colonyId', validate(createLogEntrySchema), requireColonyAccess([AccessRole.Owner, AccessRole.Collaborator]) as RequestHandler, LogController.createLog);

// Note: delete route requires both colonyId and logId.
// Example /api/logs/:colonyId/:id
router.delete('/:colonyId/:id', requireColonyAccess([AccessRole.Owner, AccessRole.Collaborator]) as RequestHandler, LogController.deleteLog);
