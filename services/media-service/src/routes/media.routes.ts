...
// Uploading requires Owner or Collaborator level on the targeted colony
router.post('/upload', validate(createUploadSchema), requireColonyAccess([AccessRole.Owner, AccessRole.Collaborator]) as RequestHandler, MediaController.requestUpload);
...
router.post('/:id/confirm', validate(confirmUploadSchema), requireColonyAccess([AccessRole.Owner, AccessRole.Collaborator]) as RequestHandler, MediaController.confirmUpload);
